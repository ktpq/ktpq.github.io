...
function showData(date) {
    if (!date) return; // หยุดการทำงานหาก date ไม่มีค่า

    let jsonEvent = localStorage.getItem(date);
    let arrayEvent = JSON.parse(jsonEvent) || []; // กำหนดให้ arrayEvent เป็น [] ถ้า jsonEvent เป็น null

    contentTodo.innerHTML = ''; // ล้างรายการก่อนแสดงใหม่

    for (let i = 0; i < arrayEvent.length; i++) {
        let imgSrc = arrayEvent[i].status === 0 ? '/image/uncheck.png' : '/image/checkbox.png';

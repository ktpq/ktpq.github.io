...
function showData(date) {
    if (!date) return; // ถ้า date ไม่มีค่า ให้หยุดการทำงาน

    let jsonEvent = localStorage.getItem(date);
    let arrayEvent = JSON.parse(jsonEvent) || []; // ใช้ fallback เป็น [] ถ้า jsonEvent เป็น null

    console.log(arrayEvent.length); // เช็คจำนวน event ที่มีในวันที่เลือก
